...
#         # Re-publish timer — keeps sending until mission confirms started
#         self._mission_started = False
#         self.create_timer(1.0, self._republish_timer_cb)
#         self.create_subscription(Bool, '/quin/mission_status', self._mission_status_cb, 10)
...
#             AB, C, DE = self.last_decoded
#             msg = Int32MultiArray()
#             msg.data = [AB, C, DE]
#             self.pub.publish(msg)
#             self.get_logger().info(
#                 f"Re-publishing mission params (tag_id={self.last_tag_id})"
#             )
...
#         self.cmd_pub        = self.create_publisher(Twist, '/quin/move_command', 10)
#         self.entry_done_pub = self.create_publisher(Bool, '/entry_done', 10)

#         self.create_subscription(Bool, '/entry_start', self._entry_start_cb, 10)

#         self.active           = False
#         self._entry_done_sent = False
#         self._last_cmd        = ""

#         self.get_logger().info("Entry Node Ready")

#     def _entry_start_cb(self, msg):
#         if not msg.data:
#             return
#         if self.active:
#             return
...
#         self._entry_done_sent = False
#         self._last_cmd        = ""
#         self.get_logger().info("Entry started — waiting for vision...")
...
#     def publish_entry_done(self, success: bool):
#         if not self._entry_done_sent:
#             msg = Bool()
#             msg.data = success
#             self.entry_done_pub.publish(msg)
#             self._entry_done_sent = True
#             self.active           = False
#             self.get_logger().info(f"Entry done published — success={success}")
...
# class EnteringThread:
#     def __init__(self, stream, mission_node: MissionPublisher, entry_node: EntryNode):
#         self.stream       = stream
#         self.ros_node     = mission_node
...
#             # ---- Sync STATE with entry_node activation ----
#             if self.entry_node.active and self.STATE == "WAITING":
#                 self.STATE        = "SEARCH_TAG"
#                 self.tag_seen     = False
#                 self.lost_counter = 0
#                 self.get_logger().info("Entry vision active — SEARCH_TAG")
...
#                     self.entry_node.send_command("ROTATE_LEFT")

#             elif self.STATE == "ALIGN_TAG":
#                 if center_x is not None:
#                     error = center_x - target_x
#                     if abs(error) < 5:
#                         self.entry_node.send_command("STOP")
#                         self.STATE = "APPROACH_TAG"
#                     elif error > 0:
#                         action_text = "TURN_RIGHT"
#                         self.entry_node.send_command("TURN_RIGHT")
#                     else:
#                         action_text = "TURN_LEFT"
#                         self.entry_node.send_command("TURN_LEFT")
...
#                     if abs(error) < 8:
#                         action_text = "FORWARD"
#                         self.entry_node.send_command("FORWARD")
#                     elif error > 0:
#                         action_text = "FORWARD_RIGHT"
#                         self.entry_node.send_command("FORWARD_RIGHT")
#                     else:
#                         action_text = "FORWARD_LEFT"
#                         self.entry_node.send_command("FORWARD_LEFT")
#                 else:
...
#                 self.entry_node.send_command("STOP")
#                 self.entry_node.publish_entry_done(True)
#                 self.STATE = "WAITING"  # reset for next mission restart

#             # ------------ GRAPHICS ------------
#             cv2.putText(draw_frame, f"ENTRY STATE: {self.STATE}", (20, 40),
#                         cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
#             cv2.putText(draw_frame, f"ACTION: {action_text}", (20, 80),
#                         cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

#             cv2.line(draw_frame, (target_x, 0), (target_x, h), (255, 0, 0), 2)

#             if center_x is not None:
#                 cv2.line(draw_frame, (center_x, 0), (center_x, h), (0, 0, 255), 2)
#                 offset = center_x - target_x
#                 cv2.putText(draw_frame, f"OFFSET: {offset}px", (20, 120),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 0), 2)
...
#     print("Starting AI threads...")
#     entering_thread = EnteringThread(stream1, mission_node, entry_node)
#     cabbage_thread  = CabbageThread(stream2, cabbage_node)
