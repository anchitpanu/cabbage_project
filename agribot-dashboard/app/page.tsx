...
import { Camera, Gauge, PlugZap, Satellite, Signal, Leaf, Ruler, Activity, Navigation, Cpu, Zap } from "lucide-react";
...
export default function Page() {
  ...
  const subscribeAll = (ros: ROSLIB.Ros) => {
    ...
    sub(TOPICS.MISSION_PARAMS, "std_msgs/msg/Int32MultiArray", (m) => {
      const d = m.data;
      if (d && d.length >= 3) { setAB(d[0]); setC(d[1]); setDE(d[2]); setParamsAt(Date.now()); }
    });
    ...
    sub(TOPICS.MOVE_COMMAND, "geometry_msgs/msg/Twist", (m) => {
      const vx = Number(m?.linear?.x ?? 0); cmdVelRef.current = vx; setCmdVelX(vx);
    }, 50);

    sub(TOPICS.CMD_MOVE,       "std_msgs/msg/Float32", (m) => { const v = Number(m?.data); if (Number.isFinite(v)) setTargetDist(v); });
    sub(TOPICS.MOVE_DONE,      "std_msgs/msg/Bool",    (m) => { setMoveDone(m?.data); setMoveDoneAt(Date.now()); });
  ...
  useEffect(() => {
    ...
    ctx.fillStyle="#1f2937"; ctx.fillRect(M.left,M.top,CW,CH);
    const pts=graph, dMax=pts.length>0?Math.max(...pts.map(p=>p.dist),1.5):1.5;
      ...
      <div className="mx-auto max-w-7xl space-y-4">
        ...
        <header className="flex items-center justify-between py-2">
          <div className="flex items-center gap-3">
            <PlugZap className={`h-6 w-6 ${isConnected?"text-green-400":"text-gray-600"}`}/>
            <div>
              <h1 className="text-2xl font-semibold text-white">AgriBot Monitor</h1>
              <p className="text-xs text-gray-500">Cabbage Field Robot — Raspberry Pi 5</p>
            </div>
          </div>
          <div className={`flex items-center gap-2 rounded-full px-3 py-1 text-sm font-medium border ${isConnected?"bg-green-950 border-green-800 text-green-400":"bg-red-950 border-red-800 text-red-400"}`}>
            <Signal className="h-4 w-4"/>{isConnected?"Connected":"Disconnected"}
          </div>
        </header>

        <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
          <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
            <Satellite className="h-4 w-4 text-blue-400"/><span>Connection</span>
          </div>
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <div className="flex items-end gap-2">
              <button onClick={connect} disabled={isConnected} className="flex-1 rounded-xl bg-blue-600 px-4 py-2 text-sm text-white font-medium hover:bg-blue-500 disabled:opacity-40 transition-colors">Connect</button>
              <button onClick={disconnect} className="flex-1 rounded-xl border border-gray-700 px-4 py-2 text-sm text-gray-400 hover:bg-gray-800 transition-colors">Disconnect</button>
            </div>
          </div>
          <label className="mt-3 flex items-center gap-2 text-sm text-gray-400 cursor-pointer">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
            <div className="mb-2 flex items-center gap-2 text-base font-semibold text-gray-200">
              <Camera className="h-4 w-4 text-blue-400"/><span>Camera — Front</span>
              <span className={`ml-auto text-xs px-2 py-0.5 rounded-full border ${isConnected?"bg-green-950 border-green-800 text-green-400":"bg-gray-800 border-gray-700 text-gray-500"}`}>
                {isConnected?"● LIVE":"● WAITING"}
              </span>
            </div>
            <div className="aspect-video w-full overflow-hidden rounded-xl border border-gray-800 bg-black">
              {isConnected?<img src={mjpeg1} alt="Front Camera" className="h-full w-full object-contain"/>
                :<div className="flex h-full items-center justify-center text-gray-600 text-sm">Connect first</div>}
            </div>
            <p className="mt-2 text-xs text-gray-600"><code className="text-gray-500">{TOPICS.FRONT_CAMERA}</code></p>
          </section>

          <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
            <div className="mb-2 flex items-center gap-2 text-base font-semibold text-gray-200">
              <Camera className="h-4 w-4 text-green-400"/><span>Camera — Top</span>
              <span className={`ml-auto text-xs px-2 py-0.5 rounded-full border ${isConnected?"bg-green-950 border-green-800 text-green-400":"bg-gray-800 border-gray-700 text-gray-500"}`}>
                {isConnected?"● LIVE":"● WAITING"}
              </span>
            </div>
            <div className="aspect-video w-full overflow-hidden rounded-xl border border-gray-800 bg-black">
              {isConnected?<img src={mjpeg2} alt="Top Camera" className="h-full w-full object-contain"/>
                :<div className="flex h-full items-center justify-center text-gray-600 text-sm">Connect first</div>}
            </div>
            <p className="mt-2 text-xs text-gray-600"><code className="text-gray-500">{TOPICS.TOP_CAMERA}</code></p>
          </section>
        </div>

        <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
          <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
            <Ruler className="h-4 w-4 text-yellow-400"/><span>AprilTag Mission Parameters</span>
            <span className="ml-auto text-xs text-gray-600">{paramsAt?"Last: "+fmtT(paramsAt):"Waiting for AprilTag..."}</span>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
            <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
              <Gauge className="h-4 w-4 text-blue-400"/><span>Distance Inside Planter</span>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              <DK icon={<Gauge className="h-4 w-4"/>}      title="Current Distance" value={fmt(distance,3)}   unit="m" sub={fmtT(distanceAt)} color="blue"   note={TOPICS.DISTANCE}/>
              <DK icon={<Navigation className="h-4 w-4"/>} title="Target Distance"  value={fmt(targetDist,3)} unit="m" sub="from mission"      color="purple" note={TOPICS.CMD_MOVE}/>
            </div>
            <div className="mb-3">
              <div className="flex justify-between text-xs text-gray-500 mb-2"><span>Progress to target</span><span className="text-gray-300 font-medium">{progress.toFixed(1)}%</span></div>
              ...
              <div className="flex justify-between text-xs text-gray-600 mt-1">
                <span>0 m</span>
                <span className={moveDone===true?"text-green-400 font-semibold":moveDone===false?"text-red-400 font-semibold":"text-gray-500"}>
            ...
            <div>
              <div className="flex justify-between text-xs text-gray-500 mb-2"><span>Commanded velocity</span><span className="text-green-400 font-medium">{fmt(cmdVelX,3)} m/s</span></div>
              <div className="h-2 rounded-full bg-gray-800 overflow-hidden">
                <div className="h-full rounded-full bg-green-500 transition-all duration-300" style={{width:cmdVelX!=null?Math.min(100,Math.abs(cmdVelX)/0.3*100)+"%":"0%"}}/>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
            <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
              <Leaf className="h-4 w-4 text-green-400"/><span>Mission &amp; Planting Status</span>
            </div>
            <div className={`rounded-xl border p-3 mb-3 ${missionStatus===true?"border-green-800 bg-green-950":missionStatus===false?"border-red-800 bg-red-950":"border-gray-700 bg-gray-800"}`}>
              <div className="text-xs text-gray-500 mb-1">Mission · <code>{TOPICS.MISSION_STATUS}</code></div>
              <div className={`text-2xl font-bold ${missionStatus===true?"text-green-400":missionStatus===false?"text-red-400":"text-gray-600"}`}>
                {missionStatus===true?"✓ COMPLETE":missionStatus===false?"✗ FAILED":"— WAITING"}
              </div>
              <div className="text-xs text-gray-600 mt-1">{fmtT(missionAt)}</div>
            </div>
            <div className={`rounded-xl border p-3 mb-3 ${plantBusy?"border-yellow-800 bg-yellow-950":plantDone===true?"border-green-800 bg-green-950":plantDone===false?"border-red-800 bg-red-950":"border-gray-700 bg-gray-800"}`}>
              <div className="text-xs text-gray-500 mb-1">Planting · <code>{TOPICS.PLANT_DONE}</code></div>
              <div className={`text-2xl font-bold ${plantBusy?"text-yellow-400":plantDone===true?"text-green-400":plantDone===false?"text-red-400":"text-gray-600"}`}>
                {plantBusy?"⟳ PLANTING...":plantDone===true?"✓ PLANTED":plantDone===false?"✗ FAILED":"— IDLE"}
              </div>
              <div className="text-xs text-gray-600 mt-1">{fmtT(plantAt)}</div>
            </div>
            <div className="rounded-xl border border-gray-700 bg-gray-800 p-3">
              <div className="text-xs text-gray-500 mb-1">Actuator ACK · <code>{TOPICS.ACTUATOR_ACK}</code></div>
              <div className={`text-xl font-bold ${actuatorAck===true?"text-green-400":actuatorAck===false?"text-red-400":"text-gray-600"}`}>
        ...
        <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
          <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
            <Cpu className="h-4 w-4 text-purple-400"/><span>ESP32 Debug — PID &amp; Encoder</span>
          </div>
          <div className="grid gap-3 md:grid-cols-4">
          ...
          {motorDuty&&(
            <div className="mt-3 grid gap-2 md:grid-cols-2">
              <div>
                <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Left Duty (PID)</span><span className="text-blue-400">{fmt(motorDuty.l,3)}</span></div>
                <div className="h-2 rounded-full bg-gray-800 overflow-hidden"><div className="h-full rounded-full bg-blue-500 transition-all" style={{width:Math.min(100,motorDuty.l*100)+"%"}}/></div>
              </div>
              <div>
                <div className="flex justify-between text-xs text-gray-500 mb-1"><span>Right Duty (PID)</span><span className="text-green-400">{fmt(motorDuty.r,3)}</span></div>
                <div className="h-2 rounded-full bg-gray-800 overflow-hidden"><div className="h-full rounded-full bg-green-500 transition-all" style={{width:Math.min(100,motorDuty.r*100)+"%"}}/></div>
              </div>
            </div>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-gray-800 bg-gray-900 p-4">
          <div className="mb-3 flex items-center gap-2 text-base font-semibold text-gray-200">
            <Activity className="h-4 w-4 text-blue-400"/><span>Movement Graph — Distance vs Time</span>
            <span className="ml-auto text-xs font-normal text-gray-600">T+{graph.length>0?graph[graph.length-1].t.toFixed(0):0}s · {graph.length} pts</span>
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-4 rounded-xl bg-gray-800 border border-gray-700 p-3 text-xs text-gray-500">
...
function DI({label,value,onChange,placeholder}:{label:string;value:string;onChange:(v:string)=>void;placeholder?:string}) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs text-gray-500">{label}</label>
      <input value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
        className="rounded-xl border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-gray-200 outline-none focus:border-blue-500 placeholder-gray-600"/>
    </div>
  );
}
...
function DK({icon,title,value,unit,sub,note,color="blue"}:{icon:React.ReactNode;title:string;value:string|number;unit?:string;sub?:string;note?:string;color?:string}) {
  const c=CM[color]??CM.blue;
  return (
    <div className={`rounded-xl border ${c.b} ${c.bg} p-3`}>
      <div className={`mb-1 flex items-center gap-1.5 text-sm font-semibold ${c.v}`}>{icon}<span className="text-gray-400">{title}</span></div>
      <div className="flex items-end justify-between gap-2">
        <div className={`text-3xl font-semibold tracking-tight ${c.v}`}>{value} {unit&&<span className="text-sm font-normal text-gray-600">{unit}</span>}</div>
        {sub&&<div className="text-xs text-gray-600">{sub}</div>}
      </div>
      {note&&<div className="mt-1 text-xs text-gray-700"><code>{note}</code></div>}
    </div>
  );
}