...
// -------- PID state --------
int32_t  pid_left_total  = 0;       // cumulative left ticks
int32_t  pid_right_total = 0;       // cumulative right ticks
float    pid_integral    = 0.0f;
float    pid_prev_error  = 0.0f;
int      pwm_counter     = 0;       // software PWM step counter
float    left_duty       = 1.0f;    // 0.0 - 1.0
float    right_duty      = 1.0f;    // 0.0 - 1.0
...
void twistCallback(const void *msgin)
{
    ...
    // so it doesn't carry over error from previous move
    resetPID();
}
...
// duty: 0.0 = always off, 1.0 = always on
// rpm: used for direction only (bang-bang)
void setMotorPWM(int a, int b, float rpm, float duty)
{
    // Determine if this cycle fires based on duty cycle
...
void driveDifferential()
{
    ...
    // ---- PID heading correction (only during straight forward/backward) ----
    // Skip PID correction during intentional turns (Wz != 0)
    if (fabsf(Wz) < 0.01f && fabsf(Vx) > 0.01f)
    {
        // error = how much left is ahead of right
        ...
        // PID output — positive = left is faster, slow it down
        float correction = constrain(P + I + D, -PID_MAX_CORR, PID_MAX_CORR);

        if (correction > 0.0f) {
            // Left side is faster → reduce left duty
            left_duty  = constrain(1.0f - correction, 0.0f, 1.0f);
            right_duty = 1.0f;
        } else {
            // Right side is faster → reduce right duty
            left_duty  = 1.0f;
    ...
    {
        // Turning or stopped — full duty, no correction
        left_duty  = 1.0f;
        right_duty = 1.0f;
    }

    // Advance software PWM counter
    pwm_counter = (pwm_counter + 1) % PWM_STEPS;

    // Apply motor commands with duty cycle
    ...
    debug_motor_msg.linear.y  = left_duty;     // now shows duty, not rpm
...
void Encoder()
{
    ...
    // Left side: M1 + M3, Right side: M2 + M4
    ...
    // Distance tracking (average of all 4)
    ...
    // Debug: raw delta ticks per motor
...
bool createEntities()
{
    ...
    // FIX: reset timeout after agent connects, not at boot
    prev_cmd_time = millis();
    resetPID();
